...
1. **指数移动平均（EMA）** —— 可以选择基于 EMA 的检查点进行最终评估
...
DeepSpeed-HE可以在训练和推理之间无缝更改模型分区，以支持`基于张量并行计算的推理`和`基于ZeRO的分片机制`进行训练。它还会重新配置内存系统以在此期间最大化内存可用性。DeepSpeed-HE还通过`规避内存分配瓶颈`和支持`Batch大小`来进一步提高性能。混合引擎集成了DeepSpeed训练和推理的一系列系统技术，突破了现有RLHF训练的极限，并为RLHF工作负载提供了无与伦比的规模和系统效率。
...
1. **有效吞吐量分析** 
	...
	在 RLHF 训练的第 3 阶段，DeepSpeed-HE 的有效吞吐量取决于它在生成和 RL 训练阶段所实现的吞吐量。
