...
def train(model, optimizer, lr_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        if iteration % 5 == 0:
            print(f"step latency: {np.mean(step_times)} | throughput: {1024 / np.mean(step_times)}")
            f.write(str(1000 / np.mean(step_times)) + "\n")
...
def get_train_val_test_data(args):
    ...
    if mpu.get_model_parallel_rank() == 0:
        if args.use_npy_data_loader:
            ...
            data_config.set_defaults(data_set_type='synthetic', transpose=False)
