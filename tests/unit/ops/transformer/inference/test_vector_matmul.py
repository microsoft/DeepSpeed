...
def run_vector_matmul_reference(input_tensor, weight):
    return torch.matmul(input_tensor, weight.transpose(0, 1)).to('cuda:0')
...
def test_vector_matmul(batch_size, in_features, out_features, dtype):
    ...
    input_tensor_ds = torch.randn(1, batch_size, in_features, dtype=dtype).to('cuda:0')
    weight_tensor_ds = torch.randn(out_features, in_features, dtype=dtype).to('cuda:0')
