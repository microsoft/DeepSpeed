...
@pytest.mark.inference
@pytest.mark.parametrize("num_groups", [1, 128])
@pytest.mark.parametrize("num_elems", [16384, 32768, 65536])
@pytest.mark.parametrize("is_symmetric_quant", [True, False])
@pytest.mark.parametrize("q_bits", [4, 8])
def test_float_quantize_large_inputs(num_elems, num_groups, is_symmetric_quant, q_bits):
