...
def test_pipe_inference_schedule_singlestage():
    ...
    assert sched.num_samples == 4
    full = list(iter(sched))
    for idx, cmds in enumerate(full):
        assert len(cmds) == 2
        assert type(cmds[0]) == schedule.LoadSample
        assert type(cmds[1]) == schedule.ForwardPass
        assert cmds[0].buffer_id == cmds[1].buffer_id
    assert len(full) == sched.num_samples
    

@pytest.mark.parametrize('samples', [1, 3, 8, 10])
def test_pipe_inference_schedule_firststage(samples, stages=3, verbose=False):
    sched = schedule.InferenceSchedule(micro_batches=samples, stages=stages, stage_id=0)
    assert sched.num_samples == samples
    full = list(iter(sched))
    if verbose:
        print()
    for idx, cmds in enumerate(full):
        ...
        assert len(cmds) == 3
        assert _count_type(cmds, schedule.LoadSample) == 1
        assert _count_type(cmds, schedule.ForwardPass) == 1
        assert _count_type(cmds, schedule.SendActivation) == 1
    assert len(full) == samples + stages - 1
    

@pytest.mark.parametrize('samples', [1, 3, 8, 10])
def test_pipe_inference_schedule_midstage(samples, stages=3, verbose=False):
    sched = schedule.InferenceSchedule(micro_batches=samples, stages=stages, stage_id=1)
    ...
    for idx, cmds in enumerate(full):
        ...
        if idx < sched.stage:
            assert len(cmds) == 0
            continue
        if idx == sched.stage + sched.num_samples:
            assert len(cmds) == 1
            assert type(cmds[0]) == schedule.SendActivation
            continue
        if idx > sched.stage + sched.num_samples:
            assert len(cmds) == 0
            continue
        assert _count_type(cmds, schedule.LoadSample) == 0
        assert _count_type(cmds, schedule.ForwardPass) == 1
...
@pytest.mark.parametrize('samples', [1, 3, 8, 10])
def test_pipe_inference_schedule_laststage(samples, stages=3, verbose=False):
    sched = schedule.InferenceSchedule(micro_batches=samples, stages=stages, stage_id=2)
    full = list(iter(sched))
    if verbose:
        print()
    for idx, cmds in enumerate(full):
        if verbose:
            print(idx, cmds)
        if idx < sched.stage or idx > sched.stage + sched.num_samples:
            assert len(cmds) == 0
            continue
        assert _count_type(cmds, schedule.LoadSample) == 1
        assert _count_type(cmds, schedule.ForwardPass) == 1
...
def test_pipe_schedule_laststage():
    ...
    #assert len(sched) == 2 * (sched.samples + sched.stages - 1)
    print()
    for cmds in sched:
        print(cmds)
