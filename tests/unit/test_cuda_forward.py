...
def run_forward(ds_config, seq_len, atol=1e-2, verbose=False, test_bsz=None):
    ...
    input_mask = torch.zeros(bsz, 1, 1, 16, **kwargs)
    input_ids = torch.arange(1, 17).view([1, 16]).to("cuda")
    print(f'input_ids: {input_ids}')
    word_embeddings = nn.Embedding(32008, 1024)
    word_embeddings.to("cuda")
    input_embeddings = word_embeddings(input_ids).half()
    print(f'input embeddings: {input_embeddings}')

    #hidden_states.data.copy_(input_embeddings.data)
...
def test_forward(batch_size,
    ...
    run_forward(ds_config, seq_len, atol=1e-3)
