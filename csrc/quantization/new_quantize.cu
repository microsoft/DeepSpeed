...
__global__ void min_max_local(const __half* vals, int total_count, __half* min, __half* max)
{
    ...
    if (id < total_count) {
        ...
        if (__hgt(data_h[0], finalMax)) finalMax = data_h[0];
        if (__hgt(data_h[1], finalMax)) finalMax = data_h[1];
        if (__hgt(data_h[2], finalMax)) finalMax = data_h[2];
        if (__hgt(data_h[3], finalMax)) finalMax = data_h[3];
        if (__hgt(data_h[4], finalMax)) finalMax = data_h[4];
        if (__hgt(data_h[5], finalMax)) finalMax = data_h[5];
        if (__hgt(data_h[6], finalMax)) finalMax = data_h[6];
        if (__hgt(data_h[7], finalMax)) finalMax = data_h[7];
        ...
        if (__hlt(data_h[0], finalMin)) finalMin = data_h[0];
        if (__hlt(data_h[1], finalMin)) finalMin = data_h[1];
        if (__hlt(data_h[2], finalMin)) finalMin = data_h[2];
        if (__hlt(data_h[3], finalMin)) finalMin = data_h[3];
        if (__hlt(data_h[4], finalMin)) finalMin = data_h[4];
        if (__hlt(data_h[5], finalMin)) finalMin = data_h[5];
        if (__hlt(data_h[6], finalMin)) finalMin = data_h[6];
        if (__hlt(data_h[7], finalMin)) finalMin = data_h[7];
    }

#pragma unroll
    for (int i = 1; i < WARP_SIZE; i <<= 1) {
        auto tempMax = g.shfl_xor(finalMax, i);
        if (__hgt(tempMax, finalMax)) finalMax = tempMax;
        auto tempMin = g.shfl_xor(finalMin, i);
        if (__hlt(tempMin, finalMin)) finalMin = tempMin;
    }
    __shared__ __half partialMax[WARP_SIZE];
    if (lane == 0) partialMax[gid] = finalMax;
    __shared__ __half partialMin[WARP_SIZE];
    if (lane == 0) partialMin[gid] = finalMin;
    b.sync();
...
#pragma unroll
    ...
    if (threadIdx.x == 0) max[group_id] = finalMax;
    if (threadIdx.x == 0) min[group_id] = finalMin;
}
                              ...
                              int num_bits)
{
    ...
    if (id < total_count) { data = vals_cast[group_index]; }

    // min_max += groups * blockIdx.x;
    while (id < groups) {
        // __half m_data = min_max[id];
        // m_data = __habs(m_data);
        if (__hgt(max[id], finalMax)) finalMax = max[id];
        if (__hlt(min[id], finalMin)) finalMin = min[id];
        id += blockDim.x;
    }
...
#pragma unroll
    ...
    float q_zero_point = ((float)finalMax - (float)finalMin) / 2 +
                         (float)finalMin;  // data-zero_point=centerized data
                                     ...
                                     int group_size)
{
    ...
    while (id < total_count) {
        data = vals[id];
        if (__hgt(data, max)) max = data;
        if (__hlt(data, min)) min = data;
        id += (blockDim.x);
    }
#pragma unroll
    for (int i = 1; i < WARP_SIZE; i <<= 1) {
        auto tempMax = g.shfl_xor(max, i);
        if (__hgt(tempMax, max)) max = tempMax;
        auto tempMin = g.shfl_xor(min, i);
        if (__hlt(tempMin, min)) min = tempMin;
    }
    __shared__ __half partialMax[WARP_SIZE];
    __shared__ __half partialMin[WARP_SIZE];
...
#pragma unroll
    ...
    float q_zero_point =
        ((float)max - (float)min) / 2 + (float)min;  // data-zero_point=centerized data
                                ...
                                int group_size)
{
    ...
    while (id < total_count / 8.0) {
        // printf("newton pointer left\n");
        // printf("id:%d, group_size:%d, total_count:%d,
        ...
        __half* data_h = reinterpret_cast<__half*>(&data);
        // #pragma unroll
        // printf("for threadIdx.x = %d, blockDim.x = %d, gridDim.x = %d, data[0] at %d is
        // %f\n",threadIdx.x,blockDim.x,gridDim.x,id,(float)(data_h[0])); printf("data at %d is
        // %f\n",blockDim.x*8*i+0,(float)(data_h[0])); printf("data at %d is
        // %f\n",blockDim.x*8*i+1,(float)(data_h[1])); printf("data at %d is
        // %f\n",blockDim.x*8*i+2,(float)(data_h[2])); printf("data at %d is
        ...
        if (__hgt((data_h[0]), max) && (id * 8 + 0 < total_count)) max = (data_h[0]);
        if (__hgt((data_h[1]), max) && (id * 8 + 1 < total_count)) max = (data_h[1]);
        if (__hgt((data_h[2]), max) && (id * 8 + 2 < total_count)) max = (data_h[2]);
        if (__hgt((data_h[3]), max) && (id * 8 + 3 < total_count)) max = (data_h[3]);
        if (__hgt((data_h[4]), max) && (id * 8 + 4 < total_count)) max = (data_h[4]);
        if (__hgt((data_h[5]), max) && (id * 8 + 5 < total_count)) max = (data_h[5]);
        if (__hgt((data_h[6]), max) && (id * 8 + 6 < total_count)) max = (data_h[6]);
        if (__hgt((data_h[7]), max) && (id * 8 + 7 < total_count)) max = (data_h[7]);

        if (__hlt((data_h[0]), min) && (id * 8 + 0 < total_count)) min = (data_h[0]);
        if (__hlt((data_h[1]), min) && (id * 8 + 1 < total_count)) min = (data_h[1]);
        if (__hlt((data_h[2]), min) && (id * 8 + 2 < total_count)) min = (data_h[2]);
        if (__hlt((data_h[3]), min) && (id * 8 + 3 < total_count)) min = (data_h[3]);
        if (__hlt((data_h[4]), min) && (id * 8 + 4 < total_count)) min = (data_h[4]);
        if (__hlt((data_h[5]), min) && (id * 8 + 5 < total_count)) min = (data_h[5]);
        if (__hlt((data_h[6]), min) && (id * 8 + 6 < total_count)) min = (data_h[6]);
        if (__hlt((data_h[7]), min) && (id * 8 + 7 < total_count)) min = (data_h[7]);
        // }
        id += (blockDim.x);
    }
                                       ...
                                       unsigned int numel_per_chunk)
{
    ...
    while (id < total_count) {
        ...
        data = vals_int[id];
        // printf("id:%d, quant_stats_idx:%d, total_count:%d,
        // blockDim.x:%d\n",id,quant_stats_idx,total_count,blockDim.x); data = vals_int[id];
                                ...
                                float* q_zero_point)
{
    ...
    // if (id == 0)
    // {
    // printf("id:%d, blockIdx.x:%d, blockDim.x:%d, threadIdx.x:%d,
    // gridDim.x:%d\n",id,blockIdx.x,blockDim.x,threadIdx.x,gridDim.x); printf("data:%f,q_scale:%f,
                              ...
                              cudaStream_t stream)
{
    ...
    dim3 grid_dim(
        groups,
        (total_count - 1) / ((group_size < threads ? group_size : threads) * 8 * groups) + 1);
                               ...
                               cudaStream_t stream)
{
    ...
    dim3 grid_dim(1);
    dim3 block_dim(1024);
    dequantize_data<<<grid_dim, block_dim, 0, stream>>>(
        vals, vals_int, total_count, q_scale_d, q_zero_point);
}
                                      ...
                                      cudaStream_t stream)
{
    ...
    dim3 block_dim(1024);
    dequantize_data_chunks<<<grid_dim, block_dim, 0, stream>>>(
        vals, vals_int, zero_point_stats, scale_stats, total_count, numel_per_chunk);
}
