                                           ...
                                           at::Tensor& alibi)
{
    ...
    auto prev_key = torch::from_blob(workspace + offset, {bsz, heads, all_tokens, k},
                                     {hidden_dim * Context::Instance().GetMaxTokenLenght(), k * Context::Instance().GetMaxTokenLenght(), k, 1},
                                     options);

    auto prev_value =
        torch::from_blob(workspace + offset + value_offset, {bsz, heads, all_tokens, k},
                         {hidden_dim * Context::Instance().GetMaxTokenLenght(), k * Context::Instance().GetMaxTokenLenght(), k, 1},
                         options);
