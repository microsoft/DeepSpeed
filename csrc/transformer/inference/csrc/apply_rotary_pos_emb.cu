...
__global__ void apply_rotary_pos_emb1(float* query_key_value,
                                     unsigned rotary_dim,
                                     unsigned seq_len,
                                     unsigned seq_offset,
                                     unsigned num_heads,
                                     ...
                                     unsigned total_count)
{
    ...
    int lane = id & 0x1f;
    unsigned head_id = blockIdx.x * MAX_WARP_NUM + gid;
    unsigned offset = (head_id % num_heads) * head_size;

    auto mixed_query = query_key_value + (head_id / num_heads) * (num_heads * head_size * 3);
    auto key_layer = mixed_query + (num_heads * head_size);
...
__global__ void apply_rotary_pos_emb1(__half* query_key_value,
                                     unsigned rotary_dim,
                                     unsigned seq_len,
                                     unsigned seq_offset,
                                     unsigned num_heads,
                                 ...
                                 cudaStream_t stream)
{
    ...
    apply_rotary_pos_emb1<<<grid_dims, block_dims, 0, stream>>>(
        query_key_value, rotary_dim, seq_len, offset, num_heads, head_size, total_count);
}

template void launch_apply_rotary_pos_emb1<float>(float*,
                                                 unsigned,
                                                 unsigned,
                                                 unsigned,
                                                 unsigned,
                                                 ...
                                                 unsigned,
                                                 cudaStream_t);
template void launch_apply_rotary_pos_emb1<__half>(__half*,
                                                  unsigned,
                                                  unsigned,
                                                  unsigned,
                                                  unsigned,
