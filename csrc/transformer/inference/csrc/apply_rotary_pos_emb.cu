...
template <typename T, int threadsPerHead>
__global__ void apply_rotary_pos_half(T* mixed_query,
                                      ...
                                      int max_out_tokens)
{
    constexpr int T_per_thread = rot_half::granularity / sizeof(T);
    ...
    if (head_idx < total_count) {
        ...
        mem_access::load_global<rot_half::granularity>(q, mixed_query + offset + base_neuron_idx);
        mem_access::load_global<rot_half::granularity>(k, key_layer + k_offset + base_neuron_idx);
...
#pragma unroll
        ...
        mem_access::store_global<rot_half::granularity>(mixed_query + offset + base_neuron_idx, q);
        mem_access::store_global<rot_half::granularity>(key_layer + k_offset + base_neuron_idx, k);
    }
}

#define LAUNCH_ROT_POS_EMB_HALF(HEAD_THREADS)                                       \
    apply_rotary_pos_half<T, HEAD_THREADS><<<grid, block, 0, stream>>>(mixed_query, \
                                                                       key_layer,   \
                                                                       rotary_dim,  \
                                                                       seq_len,     \
                                                                       offset,      \
...
void launch_apply_rotary_pos_emb(T* mixed_query,
                                 ...
                                 int max_out_tokens)
{
    constexpr int T_per_elem = rot_half::granularity / sizeof(T);
    ...
    if (threads_per_head == 4) {
        LAUNCH_ROT_POS_EMB_HALF(4);
    } else if (threads_per_head == 8) {
        LAUNCH_ROT_POS_EMB_HALF(8);
    } else if (threads_per_head == 16) {
        LAUNCH_ROT_POS_EMB_HALF(16);
    } else if (threads_per_head == 32) {
        LAUNCH_ROT_POS_EMB_HALF(32);
    }
#ifdef __HIP_PLATFORM_HCC__
    else if (threads_per_head == 64) {
        LAUNCH_ROT_POS_EMB_HALF(64);
    }
#endif
    else {
        assert(false);
