...
template <typename T>
unsigned int get_workspace_size(unsigned int maxBatchSize,
...
{
    unsigned int workSpacesize = 4 * (size_t(maxBatchSize) * seq_len * hidden_size);
    if (training) {
...
template <typename T>
BertTransformerLayer<T>::BertTransformerLayer(unsigned int layer_id,
...
template <typename T>
void BertTransformerLayer<T>::SetSeqLength(unsigned int seq_len)
{
    _seq_length = seq_len;
...
template <typename T>
int create_transformer_layer(unsigned int layer_id,
                             unsigned int batch_size,
                                                  ...
                                                  bool gelu_checkpoint)
{
    ...
    unsigned int seq_len = layer->GetSeqLength();
