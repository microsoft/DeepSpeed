                                                  ...
                                                  bool normalize_invertible)
{
    ...
    T* q_tf_ptr = (T*)qkv_tf.data_ptr();
    T* k_tf_ptr =
        q_tf_ptr + (bsz * layer->GetSeqLength() * layer->GetHiddenSize());  //(T*)k_tf.data_ptr();
    T* v_tf_ptr =
        k_tf_ptr + (bsz * layer->GetSeqLength() * layer->GetHiddenSize());  //(T*)v_tf.data_ptr();
    T* attn_o_inp_ptr = (T*)attn_o_inp.data_ptr();
                                                   ...
                                                   const torch::Tensor& norm_b)
{
    ...
    const T* add_res_ptr = (const T*)add_res.data_ptr();
    const T* k_tf_ptr =
        q_tf_ptr + (bsz * layer->GetSeqLength() * layer->GetHiddenSize());  //(const T*)k_tf.data_ptr();
    const T* v_tf_ptr =
        k_tf_ptr + (bsz * layer->GetSeqLength() * layer->GetHiddenSize());  //(const T*)v_tf.data_ptr();
    const T* ctx_bufB_ptr = (const T*)ctx_bufB.data_ptr();
    const T* soft_out_ptr = (const T*)soft_out.data_ptr();
    const T* attn_o_inp_ptr = (const T*)attn_o_inp.data_ptr();
    const T* input_mask_ptr = (const T*)input_mask.data_ptr();
                                                  ...
                                                  bool gelu_checkpoint)
{
    ...
    layer->SetTrainingMode(training_mode);
    layer->SetIntermediateBuffers((uint8_t*)layer_output_dropout_mask.data_ptr());
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("create_mlp_layer_fp32",
          ...
          "Create DeepSpeed mlp mlp Layer with fp32 (CUDA)");
    m.def("create_mlp_layer_fp16",
          &create_mlp_layer<__half>,
          "Create DeepSpeed mlp mlp Layer with fp16 (CUDA)");
}
