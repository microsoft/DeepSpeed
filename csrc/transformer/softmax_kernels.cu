...
if(threads == 1024)
{
    ...
    if(sequence_length <= 128)
        attn_softmax_v2<32, (threads / 32), 1> <<<grid_dim, block_dim, 0, stream>>>( vals, attn_mask, total_count, heads, seq_length4, reduce_threads);
    else if(sequence_length <= 256)
        attn_softmax_v2<32, (threads / 64), 2> <<<grid_dim, block_dim, 0, stream>>>( vals, attn_mask, total_count, heads, seq_length4, reduce_threads);
    else if(sequence_length <= 512)
        attn_softmax_v2<32, (threads / 128), 4> <<<grid_dim, block_dim, 0, stream>>>( vals, attn_mask, total_count, heads, seq_length4, reduce_threads);
    else if(sequence_length <= 1024)
        attn_softmax_v2<32, (threads / 128), 8> <<<grid_dim, block_dim, 0, stream>>>( vals, attn_mask, total_count, heads, seq_length4, reduce_threads);
    else if(sequence_length <= 2048)
        attn_softmax_v2<32, (threads / 128), 16> <<<grid_dim, block_dim, 0, stream>>>( vals, attn_mask, total_count, heads, seq_length4, reduce_threads);
    else if(sequence_length <= 4096)
        attn_softmax_v2<32, (threads / 128), 32> <<<grid_dim, block_dim, 0, stream>>>( vals, attn_mask, total_count, heads, seq_length4, reduce_threads);                      
    else 
