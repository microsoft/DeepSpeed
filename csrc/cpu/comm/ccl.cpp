...
void initialize(int size, int rank, torch::Tensor& kvs_data)
{
    ...
    snprintf(shm_name, NAME_BUF_SIZE, "%s_%d_%s_%s", SHM_BUFFER_NAME, getuid(), addr_string, port_string);
    ...
    if (all_ranks_local_p) {
        ...
        if (rank != 0) {
            shared_open(
                &allreduce_buffer, shm_name, size * sizeof(struct allreduce_workspace));
        }
        workspace = (struct allreduce_workspace*)allreduce_buffer.bytes;
    }
}
