...
public:
    ...
    {
        auto it = map_.begin();
        if (it == map_.end()) { return vlist_.end(); }
        return it->second;
    }
...
public:
    ...
    typedef typename std::unordered_multimap<key_t, iterator>::const_iterator const_map_it;
...
protected:
    ...
    static inline iterator find(const key_t& key) { return g_store().find(key); }

    static inline iterator end() { return g_store().end(); }
