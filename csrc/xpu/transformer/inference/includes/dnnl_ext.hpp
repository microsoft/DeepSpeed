...
public:
    ...
    {
        std::vector<query> valid_q{query::src_md,
                                   query::diff_src_md,
                                   query::weights_md,
                                   ...
                                   query::scratchpad_md,
                                   query::exec_arg_md};
        if (!std::any_of(valid_q.cbegin(), valid_q.cend(), [=](query q) { return what == q; }))
            DNNL_THROW_ERROR(dnnl_invalid_arguments, "memory descriptor query is invalid");

        const dnnl_memory_desc_t* cdesc =
            dnnl_primitive_desc_query_md(this->get_primitive_desc(), dnnl::convert_to_c(what), idx);
    ...
    const memory::desc* src_desc(int idx) const { return query_md(query::src_md, idx); }
    ...
    const memory::desc* dst_desc(int idx) const { return query_md(query::dst_md, idx); }
    ...
    const memory::desc* weights_desc(int idx) const { return query_md(query::weights_md, idx); }
    ...
    const memory::desc* diff_src_desc(int idx) const { return query_md(query::diff_src_md, idx); }
    ...
    const memory::desc* diff_dst_desc(int idx) const { return query_md(query::diff_dst_md, idx); }
    ...
    ///     workspace parameter.
    const memory::desc* workspace_desc() const { return query_md(query::workspace_md, 0); }
    ...
    const memory::desc* scratchpad_desc() const { return query_md(query::scratchpad_md, 0); }
};
}  // namespace dnnl
