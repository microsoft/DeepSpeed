...
#pragma unRoll
        for (int i = 0; i < unRoll; i++) {
            ...
            mem_access::load_global<ln::granularity>(iteration_buffer,
                                                     input_base + i * stride,
...
#pragma unRoll
            for (int j = 0; j < T_per_load; j++) {
                ...
                val = (val - mean) * denom;
                val = val * conversion::to<float>(gamma_local[j]) +
                      conversion::to<float>(beta_local[j]);
...
template void launch_fused_ln(sycl::half*,
                              ...
                              sycl::queue);
template void
launch_fused_ln(bf16*, const bf16*, const bf16*, const bf16*, float, int, int, sycl::queue);
template void
