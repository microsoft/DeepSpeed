...
public:
    fused_bias_gelu(T* input, const T* bias, int total_count, int intermediate_size)
        : input(input),
          bias(bias),
    ...
    void operator()(sycl::nd_item<1> pos) const
    {
        ...
        const int offset =
            (pos.get_group(0) * pos.get_local_range(0) + pos.get_local_id(0)) * values_per_access;
...
public:
    fused_bias_add(T* input, const T* bias, int total_count, int intermediate_size)
        : input(input),
          bias(bias),
          total_count(total_count),
          intermediate_size(intermediate_size){};
...
public:
    fused_bias_residual(T* residual,
                        const T* hidden_state,
                        const T* attn,
                        const T* bias,
                        const T* attn_bias,
    ...
    void operator()(sycl::nd_item<1> pos) const
    {
        using T2 = typename std::conditional<std::is_same<T, half>::value, half2, float2>::type;
...
public:
    fused_bias_residual(float* residual,
                        const float* hidden_state,
                        const float* attn,
                        const float* bias,
                        const float* attn_bias,
                          ...
                          sycl::queue stream)
{
    ...
    fused_bias_residual<T> fn(residual,
                              hidden_state,
                              attn,
                              bias,
                              attn_bias,
...
template void launch_bias_residual<
    float>(float*, float*, float*, float*, float*, int, int, int, bool, sycl::queue);
template void
launch_bias_residual<bf16>(bf16*, bf16*, bf16*, bf16*, bf16*, int, int, int, bool, sycl::queue);
template void
launch_bias_residual<half>(half*, half*, half*, half*, half*, int, int, int, bool, sycl::queue);
