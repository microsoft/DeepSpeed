...
void BitInterleaving_2bit(unsigned char* PTR_4Bytes)
{
    ...
    //
    //int order_2bit[16] = {1,5,9,13,3,7,11,15,2,6,10,14,4,8,12,16};  // pre-defined order for bit-interleaving in QuantLLM
 ...
 * 8 FP2 = 2 Bytes
 */
void GenMatrix_Weight_FP6(unsigned char *Weight_6bit, unsigned char *Weight_2bit, unsigned char *Weight_4bit, size_t M, size_t K)
{
    assert(M % 64 == 0);
    assert(K % 64 == 0);
    //
...
*/
void GenMatrix_Scale_FP16(unsigned char *Scale_Out, unsigned char *Scale_In, size_t M, size_t K, int GroupSize)
{
    ...
    for(size_t i=0; i<M/64; i++)
        for(size_t j=0; j<K/GroupSize; j++)
            for(int l=0; l<8; l++)
            {   
                *Out_PTR = GetShort(Scale_In, 0 +64*i+l, j, BytesPerRow); Out_PTR += 1;
                *Out_PTR = GetShort(Scale_In, 8 +64*i+l, j, BytesPerRow); Out_PTR += 1;
                *Out_PTR = GetShort(Scale_In, 16+64*i+l, j, BytesPerRow); Out_PTR += 1;
                *Out_PTR = GetShort(Scale_In, 24+64*i+l, j, BytesPerRow); Out_PTR += 1;
                *Out_PTR = GetShort(Scale_In, 32+64*i+l, j, BytesPerRow); Out_PTR += 1;
                *Out_PTR = GetShort(Scale_In, 40+64*i+l, j, BytesPerRow); Out_PTR += 1;
                *Out_PTR = GetShort(Scale_In, 48+64*i+l, j, BytesPerRow); Out_PTR += 1;
                *Out_PTR = GetShort(Scale_In, 56+64*i+l, j, BytesPerRow); Out_PTR += 1;
            }
    return;
}