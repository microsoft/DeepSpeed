...
from deepspeed.checkpoint.constants import OPTIMIZER_STATE_DICT, FP32_FLAT_GROUPS, PARTITION_COUNT, ZERO_STAGE, \
    GROUP_PARTITION_COUNT, PARO_STRATEGY
from deepspeed.ops.adam import DeepSpeedCPUAdam
from deepspeed.runtime import ZeROOptimizer
from deepspeed.runtime.comm.coalesced_collectives import reduce_scatter_coalesced, reduce_scatter_coalesced_with_padding
...
class ParoIIGOptimizer(ZeROOptimizer):
    """
    ...
    1. params grads 节点内shard 节点间replica
    2. optimizer 进行全局shard
    ...
    def __init__(self,
        ...
        see_memory_usage("Paro IIG initialize beginning", force=True)
        ...
        self.rank = dist.get_rank(group=self.dp_process_group)
        ...
        if not self.offload_optimizer and self.is_gradient_accumulation_boundary:
            for i, sub_group in enumerate(self.fp16_groups):
                self.averaged_gradients[i] = [
                    self.__param_id_to_grad_partition[param.ds_id].narrow(0, 0, param.zero4_os_partition_numel())
                    if param.requires_grad else torch.zeros_like(param.ds_os_tensor) for param in sub_group
                ]
    ...
    def __reduce_and_partition_ipg_grads(self, safe_mode: bool = False) -> None:
        ...
        with get_accelerator().stream(self.reduce_and_partition_stream):
            ...
            grads = self.partition_grads(self.params_in_ipg_bucket, grad_partitions)

            if self.is_gradient_accumulation_boundary and self.num_nodes > 1:
                grads = reduce_scatter_coalesced(grads, self.zero4_group[f'global_{groups._get_current_device()}'])
                for param, grad_partition in zip(self.params_in_ipg_bucket, grads):
    ...
    def __avg_scatter_grads(self, params_to_reduce: List[Parameter]) -> List[Tensor]:
        ...
        grad_partitions_for_rank = reduce_scatter_coalesced_with_padding(full_grads_for_rank,
                                                                         self.zero4_group[f'local_{self.node_rank}'])
