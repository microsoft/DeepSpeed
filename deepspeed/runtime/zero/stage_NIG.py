...
class ParoNIGOptimizer(ZeROOptimizer):
    """
    ...
    def __init__(self,
        ...
        # only support the intra gradient accumulation
        self.gradient_accumulation_steps = 1
        # we perform the gradient accumulation before the inter-node communication
        self.intra_gradient_accumulation_steps = gradient_accumulation_steps
        self.cur_intra_gradient_accumulation_steps = 0
        ...
        # There are the offsets to differentiate each node's grads in the copy_buffer_to_graidents tensor in the same replicate group.
        self.group_offsets = []

        # These are the parameters in each group that will not be updated by this process directly
        self.params_not_in_partition = []
        self.group_params_not_in_partition = []

        # These are the parameters that will be updated by this process directly
        self.params_in_partition = []
        self.group_params_in_partition = []

        # Offset from the first parameter in the the self.params_in_partition
        # the parameter boundaries may not align with partition boundaries
        # so we need to keep track of the offset
        self.first_offset = []
        self.group_first_offset = []

        # number of elements per partition in each group
        self.partition_size = []
        self.group_partition_size = []

        # align nccl all-gather send buffers to 4-byte boundary
        self.nccl_start_alignment_factor = 2  # 4-byte alignment/sizeof(fp16) = 2

        self.zero21_group = groups._get_local_all_to_all_group()
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            nodewise_partition_size = []
            nodewise_params_in_partition = [None] * self.node_size
            nodewise_params_not_in_partition = [[] for _ in range(self.node_size)]
            nodewise_first_offset = []

            # generate the group_params_in_partition
            for node_rank in range(self.node_size):
                new_partition_id = self.local_rank_in_local_group + node_rank * self.local_size_in_local_group
                params_in_partition, _, first_offset = self.get_partition_info(
                    self.round_robin_bit16_groups[i], partition_size, new_partition_id)
                nodewise_partition_size.append(partition_size)
                nodewise_params_in_partition[node_rank] = params_in_partition
                nodewise_first_offset.append(first_offset)

            self.group_partition_size.append(nodewise_partition_size)
            self.group_params_in_partition.append(nodewise_params_in_partition)

            # generate the group_params_not_in_partition
            for param in self.round_robin_bit16_groups[i]:
                for group_index, params_in_partition in enumerate(self.group_params_in_partition[i]):
                    in_the_certain_node = False
                    for group_param in params_in_partition:
                        if param is group_param:
                            in_the_certain_node = True

                    if not in_the_certain_node:
                        nodewise_params_not_in_partition[group_index].append(param)
            self.group_params_not_in_partition.append(nodewise_params_not_in_partition)
            self.group_first_offset.append(nodewise_first_offset)
        ...
        self.is_param_in_current_group_partition = {rank: {} for rank in range(self.node_size)}
        self.is_param_in_current_merged_group_partition = {}
        ...
        for param_groups in self.group_params_in_partition:
            for group_index, param_group in enumerate(param_groups):
                for param in param_group:
                    self.is_param_in_current_group_partition[group_index][self.get_param_id(param)] = True
                    self.is_param_in_current_merged_group_partition[self.get_param_id(param)] = True

        for param_groups in self.group_params_not_in_partition:
            for group_index, param_group in enumerate(param_groups):
                for param in param_group:
                    self.is_param_in_current_group_partition[group_index][self.get_param_id(param)] = False
                    if not self.is_param_in_current_merged_group_partition.get(self.get_param_id(param), False):
                        self.is_param_in_current_merged_group_partition[self.get_param_id(param)] = False
        ...
        self.avg_grads_in_partition = None
    ...
    @property
    def is_intra_gradient_accumulation_boundary(self):
        if self.cur_intra_gradient_accumulation_steps >= self.intra_gradient_accumulation_steps:
            return True
        return False
    ...
    def get_zero21_process_group(self):
        """
        'local_{local_rank_localG}'
        'global_{local_rank_globalG}'
        """
        return groups._get_local_all_to_all_group()

    @property
    def rank(self):
        return dist.get_rank(group=self.dp_process_group)

    @property
    def local_size_in_local_group(self):
        return get_accelerator().device_count()

    @property
    def local_rank_in_local_group(self):
        return self.rank % self.local_size_in_local_group

    @property
    def local_size_in_global_group(self):
        return self.node_size

    @property
    def local_rank_in_global_group(self):
        return self.rank // self.local_size_in_local_group

    @property
    def node_size(self):
        return dist.get_world_size(group=self.dp_process_group) // get_accelerator().device_count()

    @property
    def local_group_id(self):
        return self.node_rank

    @property
    def global_group_id(self):
        return self.local_rank_in_local_group

    @property
    def node_rank(self):
        return self.rank // get_accelerator().device_count()
    ...
    def independent_gradient_partition_epilogue(self):
        ...
        tensor_partition_flat_buffer = self.reduce_ipg_grads(is_return=True)
        self.report_ipg_memory_usage(f"In ipg_epilogue after reduce_ipg_grads", 0)

        inter_group = self.get_zero21_process_group()[f'global_{self.global_group_id}']
        global_world_sz = self.node_size
        self.cur_intra_gradient_accumulation_steps += 1

        if self.avg_grads_in_partition is None:
            self.avg_grads_in_partition = tensor_partition_flat_buffer
        else:
            self.avg_grads_in_partition.add_(tensor_partition_flat_buffer)
            tensor_partition_flat_buffer = None

        if self.is_intra_gradient_accumulation_boundary:

            offsets_lst = self.group_offsets[:-1]
            tensor_partition_buffer_for_each_rank = list(torch.tensor_split(self.avg_grads_in_partition, offsets_lst))
            assert dist.get_world_size(group=inter_group) == self.node_size

            # batched reduce-scatter call
            dist.reduce_scatter(output=tensor_partition_buffer_for_each_rank[self.local_rank_in_global_group],
                                input_list=tensor_partition_buffer_for_each_rank,
                                group=inter_group,
                                async_op=False)
    ...
    def average_tensor(self, tensor):
        ...
        with get_accelerator().stream(stream):
            ...
            for i, (dst, bucket_offset, numel) in enumerate(rank_and_offsets):
                ...
                # dist.barrier()
                # dist.barrier()
                dst_rank = dst % self.local_size_in_local_group + self.node_rank * self.local_size_in_local_group
                async_handle = dist.reduce(grad_slice, dst=dst_rank,
                                           group=self.get_zero21_process_group()[f'local_{self.local_group_id}'],
                                           async_op=True)
    ...
    def copy_grads_in_partition(self, param):
        ...
        if self.grads_in_partition is None:
            self.grads_in_partition_offset = 0
            total_size = 0
            for group_index, group_params in enumerate(self.group_params_in_partition):
                for params_lst in group_params:
                    for t_param in params_lst:
                        total_size += t_param.numel()
        ...
        new_grad_tensor.copy_(param.grad.view(-1))
        # avoid the param.grad.data is overwritten
        if not self.avoid_written:
            param.grad.data = new_grad_tensor.data.view_as(param.grad)
        self.grads_in_partition_offset += param.numel()

    def reduce_ipg_grads(self, is_return=False):
        if self.contiguous_gradients:
        ...
        self.param_in_current_group_partition_cnt = {}
        self.avoid_written = False

        with get_accelerator().stream(stream):
            self.group_offsets = []
            for group_index in range(self.node_size):
                for _, param, param_id in self.params_in_ipg_bucket:
                    ...
                    # self.params_already_reduced[param_id] = True  # may go wrong
                    if self.partition_gradients:
                        if not self.is_param_in_current_merged_group_partition[param_id]:
                            if self.overlap_comm and self.contiguous_gradients is False:
                                raise NotImplemented
                        elif self.contiguous_gradients:
                            if self.is_param_in_current_group_partition[group_index][param_id]:
                                self.param_in_current_group_partition_cnt[
                                    param_id] = self.param_in_current_group_partition_cnt.get(param_id, 0) + 1
                                # The nodes in same replicate group may own the same params.grad and
                                # there are multiple identical params belonging to different ranks in the buffer.
                                # Therefore, grad.data should point to the corresponding tensor in the
                                # buffer(self.grads_in_partition); otherwise wrong tensor is updated.
                                if self.param_in_current_group_partition_cnt[param_id] >= 2:
                                    if self.param_in_current_group_partition_cnt[
                                        param_id] > self.local_rank_in_global_group + 1:
                                        self.avoid_written = True
                                self.copy_grads_in_partition(param)
                                self.avoid_written = False
                    else:  # zero stage 1 - partition only optimizer state
                        raise NotImplemented
                self.group_offsets.append(self.grads_in_partition_offset)
        ...
        if is_return:
            return self.grads_in_partition
        #####################################################################
