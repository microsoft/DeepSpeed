...
class PartitionedParameterCoordinator:
    ...
    def record_trace(self, sub_module: Module) -> None:
        ...
        p_ids = [p.ds_id for p in iter_params(sub_module)]
        print(f'record_trace step: {self.__step_id} mod_id: {sub_module.id} = {p_ids}')
    ...
    def fetch_sub_module(self, current_submodule: Module) -> None:
        ...
        def _dump_p_ids(tag, mod_id, param_list):
            ids = [p.ds_id for p in param_list]
            print(f'{tag} mod {mod_id} {ids}')
        ...
        _dump_p_ids('params_to_fetch', current_submodule.id, params_to_fetch)
        ...
        if self.trace_complete:
            ...
            if self.__prefetch_bucket_sz > 0:
                ...
                _dump_p_ids('params_to_prefetch', current_submodule.id, params_to_prefetch)
    ...
    def release_sub_module(self, submodule: Module) -> None:
        ...
        for param in iter_params(submodule):
            ...
            if param.ds_id in params_to_release and not param.is_external_param:
                ...
                released_params.append(param.ds_id)
        print(f'release_params mod {submodule.id} = {released_params}')
    ...
    def __prefetch_nvme_param_partitions(self) -> None:
        ...
        for param_in_trace in self.__param_queue:
            ...
            if param.ds_tensor.status == PartitionedParamStatus.NOT_AVAILABLE:
                print(f'swap_in_param append param = {param.ds_id}, numel = {param.ds_numel}, buffer count = {param.nvme_swapper.available_swap_in_buffers()}')
        ...
        if swap_in_params:
            ...
            prefetch_ids = [p.ds_id for p in swap_in_params]
            print(f'nvme_prefetch = {prefetch_ids}')
