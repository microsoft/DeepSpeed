...
class ParoNIIOptimizer(ZeROOptimizer):
    """
    ...
    def __init__(self,
        ...
        self.zero20_group = groups._get_local_all_to_all_group()
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            partition_id = self.local_rank_in_local_group
            ...
            # 这里直接对flat tensor按group_size分组
            # 关注下边界问题
            partition_size = len(self.bit16_groups_flat[i]) / self.local_size_in_local_group
    ...
    def get_zero20_process_group(self):
        """
    ...
    def average_tensor(self, tensor):
        ...
        with get_accelerator().stream(stream):
            ...
            if not self.ipg_bucket_has_moe_params:
                ...
                print(f'group size {dist.get_world_size(group=self.dp_process_group)}')
            ...
            for i, (dst, bucket_offset, numel) in enumerate(rank_and_offsets):
                ...
                async_handle = dist.reduce(grad_slice, dst=dst_rank,
                                           group=self.get_zero20_process_group()[f'local_{self.local_group_id}'],
                                           async_op=True)
    ...
    def copy_grads_in_partition(self, param):
        if self.cpu_offload:
            self.print_rank_0('[copy_grads_in_partition] self.gradient_accumulation_steps: {}'.format(
                self.gradient_accumulation_steps))
            if self.gradient_accumulation_steps > 1:
                self.async_accumulate_grad_in_cpu_via_gpu(param)

            if self.is_gradient_accumulation_boundary:
                self.print_rank_0('This is self.is_gradient_accumulation_boundary: {} '.format(
    ...
    def get_data_parallel_partitions(self, tensor, group_id):
        ...
        dp = self.local_size_in_local_group
    ...
    def get_grad_norm_direct(self, gradients, params, norm_type=2):
        ...
        if norm_type == inf:
            ...
            total_norm_cuda = get_accelerator().FloatTensor([float(total_norm)])
            dist.all_reduce(total_norm_cuda, op=dist.ReduceOp.MAX,
                            group=self.get_zero20_process_group()[f'local_{self.local_group_id}'])
    ...
    def step(self, closure=None):
        ...
        for i, group in enumerate(self.bit16_groups):
            ...
            if self.cpu_offload:
                ...
                if self.node_size > 1:
                    # TODO: 目前phi/6实验，应该不大。后续可以bucket分块，避免太大。
                    inter_group = self.get_zero20_process_group()[f'global_{self.global_group_id}']
                    assert dist.get_world_size(group=inter_group) == self.node_size
                    dist.all_reduce(single_grad_partition, group=inter_group)
        ...
        # all_gather param here, 使用inner group进行allgather
        all_gather_dp_groups(partitioned_param_groups=self.parallel_partitioned_bit16_groups,
                             dp_process_group=[self.get_zero20_process_group()[f'local_{self.local_group_id}'] for i in
