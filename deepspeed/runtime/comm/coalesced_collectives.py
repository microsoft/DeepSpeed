...
@instrument_w_nvtx
@torch.no_grad()
def all_to_all_loco_quant_reduce(params: List[Tensor],
                                 groups: {},
                                 loco_param: Any = None,
) -> List[Tensor]:
    global quantizer_module
    ...
    for idx, p in enumerate(params):
        ...
        if tensor.dim() == 1:
            ...
            if not hasattr(p, 'intra_ef_buf') or loco_idx > reset_T:
                ...
                inter_err = torch.zeros(tensor.numel()//local_world_size,
                                        device = tensor.device, dtype = tensor.dtype)
            else:
                intra_err = quantizer_module.dequantize(p.intra_ef_buf[0], p.intra_ef_buf[1], p.intra_ef_buf[1].numel(),
                                                            8, quantizer_module.Symmetric)
                inter_err = quantizer_module.dequantize(p.inter_ef_buf[0], p.inter_ef_buf[1], p.inter_ef_buf[1].numel(),
                                                            8, quantizer_module.Symmetric)
            ...
            p.intra_ef_buf = quantizer_module.quantize(intra_err, intra_quant_group, 8,
                                                   quantizer_module.Symmetric)

            global_input_tensor, global_scales = quantizer_module.loco_quantized_reduction(
                local_output, scale_output, inter_err, err_beta,
                intra_quant_group, inter_quant_group, 4, quantizer_module.Symmetric,
                local_world_size)
            ...
            p.inter_ef_buf = quantizer_module.quantize(inter_err, inter_quant_group, 8,
                                                            quantizer_module.Symmetric)
