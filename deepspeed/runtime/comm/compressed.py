...
class CompressedBackend(object):
    ...
    def compressed_allreduce(self, buffer_m: torch.tensor, worker_error, server_error, local_rank):
        ...
        worker_scale = torch.linalg.vector_norm(buffer_m) / np.sqrt(torch.numel(buffer_m))
        ...
        server_scale = torch.linalg.vector_norm(compensated_server_m) / np.sqrt(compensated_server_m.numel())
