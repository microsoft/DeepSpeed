...
def batch_by_size(
    ...
    batch_sizes, batch_sum_metrics, batch_max_metrics, microbatch_sample_ids = [], [], [], []
    for rank in range(0, len(microbatches), num_microbatches_per_batch):
        ...
        batch_sum_metric = sum([m[0] for m in microbatch[0]])
        batch_max_metric = max([m[0] for m in microbatch[0]])
        batch_sample_ids = [[m[1] for m in metrics] for metrics in microbatch]
        batch_sizes.append(batch_size)
        batch_sum_metrics.append(batch_sum_metric)
        batch_max_metrics.append(batch_max_metric)
        microbatch_sample_ids += batch_sample_ids
        if verbose:
            print(f"Batch size {batch_size} samples, metric value {batch_sum_metric}, samples: {batch_sample_ids}")
    ...
    return microbatch_sample_ids, batch_sizes, batch_sum_metrics, batch_max_metrics
...
def get_dataloader_and_lr_scheduler_for_variable_batch_size(
    ...
    # So we need to make sure batch size remains constant across all microbatches in a batch.
    microbatch_sample_ids, batch_sizes, batch_sum_metrics, batch_max_metrics = batch_by_size(
    ...
    lr_scheduler = lr_scheduler_for_variable_batch_size(base_batch_size=base_batch_size,
                                                        ...
                                                        batch_metrics=batch_sum_metrics,
...
if __name__ == "__main__":
    ...
    for epoch in range(2):
        for sample_idx, (inputs, labels) in enumerate(dataloader):
            ...
            if dataloader_rank == 0:
                print(
                    f"batch {batch_id}, batch {batch_id}, loss {loss.item()}, LRs {lr_scheduler.get_lr()}, epoch {epoch}"
                )
            engine.step()
