...
@triton.jit
def _bias_add_silu(Mataddr,  # *Pointer* to first input vector.
...
@triton.jit
def _bias_add_relu(Mataddr,  # *Pointer* to first input vector.
               biasaddr,  # *Pointer* to second input vector.
               output_ptr,  # *Pointer* to output vector.
               stride_mat,
               M, N,    # Size of the vector.
...
@triton.jit
def _bias_add_gelu(Mataddr,  # *Pointer* to first input vector.
               biasaddr,  # *Pointer* to second input vector.
               output_ptr,  # *Pointer* to output vector.
               stride_mat,
               M, N,    # Size of the vector.
...
def bias_add_act(x: torch.Tensor, y: torch.Tensor, act: str)-> torch.Tensor:
    ...
    if act == 'gelu':
        ...
        _bias_add_gelu[(grid)](x, y, output, 
                               x.stride(0), M, N,
                               sqrt, mulparam)
    elif act == 'relu':
        _bias_add_relu[(grid)](x, y, output,
                               x.stride(0), M, N)
    elif act == 'silu':
        _bias_add_silu[(grid)](x, y, output, 
                               x.stride(0), M, N)
