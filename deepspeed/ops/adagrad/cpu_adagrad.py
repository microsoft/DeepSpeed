...
class DeepSpeedCPUAdagrad(torch.optim.Optimizer):
  ...
  def __init__(self,
    ...
    self.ds_opt_adagrad.create_adagrad(self.opt_id, lr, eps, weight_decay,
                                       should_log_le("info"))
    ...
    for group_id, group in enumerate(self.param_groups):
      for param_id, p in enumerate(group['params']):
        ...
        if p.grad.is_sparse == True:
          ...
          self.ds_opt_adagrad.adagrad_update(self.opt_id, state['step'],
                                             group['lr'], group['eps'],
                                             group['weight_decay'],
                                             sparse_param.values(),
                                             p.grad.values(),
          ...
          if fp16_param_groups is not None:
            self.ds_opt_adagrad.adagrad_update_copy(
                self.opt_id, state['step'], group['lr'], group['eps'],
